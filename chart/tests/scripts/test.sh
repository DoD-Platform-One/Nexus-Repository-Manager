...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -m 5 "http://${docker_host}/v2/")
...
if skopeo login --tls-verify=false --authfile=/test/auth.json ${docker_host} -u ${docker_user} -p ${docker_password}; then
